...
mongoose.connect(dbURI, (err, db) => {
  db.dropDatabase()
    .then(() => Project.create([{
      ...
      description: 'My final project on the course was completed in a group of three, using React for the first time. This web app is designed to be a marketplace for buying, selling or looking at tickets for a wide range of global events. The app included API’s from Paypal for payment, MailChimp Subscription Service to send out newsletters and Google Map with Autocomplete for event location. The Ticketmaster and EventBrite API’s are also implemented if the event that you are looking for is currently not available on the app. My first collaborative project as Project Manage. Given the amount accomplished in the week I look forward to taking up the offer in future collaborative projects.',
